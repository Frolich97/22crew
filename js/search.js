$(function(){
    $.getJSON('json/zaradie.json', function(data) {
            for(var i=0;i<data.complexs.length;i++){
                $('#complexs').append('<option value="' + data.complexs[i].complex + '">' + data.complexs[i].complex + 
                '</option>');
            }
    });
});
...
for (var i = document.getElementById('result').getElementsByTagName('tr').length -1; i; i--) {
	...
	$.getJSON('json/zaradie.json', function(data) {
			...
			if(data.tours[i].complex == $("#complexs").val()){
                $('#result').append('<tr><td>' + data.tours[i].price + '</td><td>' + data.tours[i].arrival+'</td></tr>');
			}
			} 
    });	
    };
